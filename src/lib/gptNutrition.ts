...
export async function callGptNutritionFallback(
  ...
  console.time(`⏱️ [${requestId}] GPT nutrition fallback`);
  console.log(`🤖 [${requestId}] Calling GPT nutrition fallback for: ${foodDescription}`);
  ...
  try {
    ...
    console.log(`⏳ [${requestId}] Waiting for GPT nutrition response (timeout: ${timeoutMs}ms)`);
    ...
    console.log(`⌛ [${requestId}] GPT nutrition response received in ${endTime - startTime}ms`);
    ...
    if (response) {
      ...
      try {
        ...
        try {
          ...
          console.error(`❌ [${requestId}] Invalid JSON response from GPT:`, responseText);
          nutritionData = {};
        }
        ...
        if (!hasValidNutritionData) {
          console.warn(`⚠️ [${requestId}] GPT returned incomplete nutrition data, using fallback values`);
        } else {
          console.log(`✅ [${requestId}] Successfully generated nutrition data with GPT`);
        }
    ...
    console.error(`❌ [${requestId}] Error in GPT nutrition fallback:`, errorMessage);
    
    if (errorMessage.includes('timed out')) {
      console.error(`⏱️ [${requestId}] GPT nutrition request timed out after ${timeoutMs}ms`);
    }
  }
  
  console.log(`⚠️ [${requestId}] Using default nutrition values due to GPT fallback error`);
  console.timeEnd(`⏱️ [${requestId}] GPT nutrition fallback`);
