// apply-cors.js - Apply CORS configuration using Firebase Admin SDK
const admin = require('firebase-admin');
const fs = require('fs');

// Load configuration from firebase/cors.json
let corsConfig;
try {
  corsConfig = JSON.parse(fs.readFileSync('./firebase/cors.json', 'utf8'));
  console.log('Loaded CORS configuration from firebase/cors.json:');
  console.log(JSON.stringify(corsConfig, null, 2));
} catch (error) {
  console.error('Error loading CORS configuration:', error);
  process.exit(1);
}

// Initialize Firebase Admin with the credentials from .env.local
try {
  admin.initializeApp({
    credential: admin.credential.cert({
      projectId: "snaphealth-39b14",
      clientEmail: "firebase-adminsdk-fbsvc@snaphealth-39b14.iam.gserviceaccount.com",
      privateKey: "-----BEGIN PRIVATE KEY-----\\nDNl9WzJgRRbFKlHwKqVaweKw\n-----END PRIVATE KEY-----\n"
    }),
    storageBucket: "snaphealth-39b14.appspot.com"
  });
  console.log('Firebase Admin SDK initialized successfully');
} catch (error) {
  console.error('Error initializing Firebase Admin SDK:', error);
  process.exit(1);
}

// Get a reference to the storage bucket
const bucket = admin.storage().bucket();

// Apply CORS configuration
console.log('Applying CORS configuration to bucket: snaphealth-39b14.appspot.com');
bucket.setCorsConfiguration(corsConfig)
  .then(() => {
    console.log('✅ CORS configuration applied successfully!');
    
    // Verify the configuration
    return bucket.getCorsConfiguration();
  })
  .then((config) => {
    console.log('Current CORS configuration:');
    console.log(JSON.stringify(config, null, 2));
    
    console.log('\n✅ CORS configuration has been updated to allow access from:');
    config[0].origin.forEach(origin => {
      console.log(`  - ${origin}`);
    });
    
    process.exit(0);
  })
  .catch((error) => {
    console.error('❌ Error applying CORS configuration:', error);
    process.exit(1);
  }); 